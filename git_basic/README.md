...
2. Настройка имени и почты пользователя от которого отправляются изменения

```git
git config --global user.name "<ВАШЕ ИМЯ ПРОФИЛЯ GITHUB>"
git config --global user.email "<ВАШЕ ПОЧТА ПРОФИЛЯ GITHUB>"
```

3. Проверка статуса текущей рабочей копии

```git
git status
```

4. Добавление изменений и удаленных или новых файлов в отслеживаемые

```git
git add .
```

5. Оставить коммит с сообщением

git commit -m "<СООБЩЕНИЕ КОММИТА>"

6. Стянуть последние изменения из удаленного репозитория с ветки `main`

```git
git pull origin main
```

7. Отправить изменения на удаленный репозиторий в ветку `main`

```git
git push origin main
```

8. Указать в какой ветке пользователь сейчас находится

```git
git branch
```

9. Посмотреть всю историю коммитов

```git
git log
```

10. Посмотреть историю коммитов текущей ветки в сжатом виде

```git
git log --oneliner
```

11. Сделать ветку от выбранного коммита из истории и переключится на нее.

```git
git checkout -b feature 987651
```
...
13. Слияние веток `main` и feature

```git
git merge main feature
```

14. Для реализации открытой модели разработки, когда участники проекта делают реплику (форк) ветки основного репозитория и вносят изменения в ветках своей копии репозитория. Готовые изменения направляются владельцу исходного репозитория с помощью `pull request`.
